...
class Host(object):
    ...
    def from_hostname(host_class, name):
        ...
        if not host_info:
            module_logger.error("No hosts found fitting name " "{:}".format(name))
            return None

        if len(host_info) > 1:
            module_logger.error(
                "Multiple hosts fitting name {:}, "
    ...
    def ping(self, wait=1):
        ...
        ping_response = subprocess.call(
            ["ping", "-c1", "-w{:}".format(wait), "{:}".format(self.name)],
    ...
    def __repr__(self):
        return "<{:} with IP {:} and Ethernet Address {:}".format(
            self.name, self.ip, self.ethernet_address
        )
...
class HostGroup(object):
    ...
    def __init__(self, hosts):
        self._hosts = dict(
            [(node, Host(node, node_info)) for node, node_info in hosts.items()]
        )
    ...
    def ping_group(self, wait=1):
        ...
        """
        pings = [
            (name, self._hosts[name].ping(wait=1))
