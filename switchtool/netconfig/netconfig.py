...
class NetConfig(object):
    ...
    def load_nodes(self, infer_location=False):
        ...
        raw = subprocess.check_output(
            ["/reg/common/tools/bin/netconfig", "search", "*"]
        ).decode("utf-8")
        ...
        for device, info in self._nodes.items():
            try:
                ...
                module_logger.debug(
                    "{:} has no ethernet " "address listed".format(device)
                )
        ...
        if infer_location:
            ...
            module_logger.debug(
                "Parsed netconfig information for " "relevant location phrases"
            )
    ...
    def find_hosts(self, key, as_list=False, as_dict=False, as_object=False):
        ...
        if not any([as_list, as_dict, as_object]):
            module_logger.debug(
                "No output mode selected, so by default " "will return as a dictionary"
            )
            as_dict = True
        ...
        if as_object:
            if len(matches) > 1:
                ...
                return host.Host(
                    list(match_info.keys())[0], list(match_info.values())[0]
                )
    ...
    def search(self, as_list=False, as_dict=False, as_object=False, **kwargs):
        ...
        for node, host_info in hosts.items():
            for param, search_param in kwargs.items():
                ...
                if not match_value or not fnmatch.filter(
                    [match_value.lower()], search_param.lower()
                ):
                    del hosts[node]
                    break

        module_logger.debug(
            "Found {:} matches for search parameters".format(str(len(hosts)))
        )
